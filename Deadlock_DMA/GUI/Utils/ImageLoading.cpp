...
bool LoadTextureFromMemory(const void* data, size_t data_size, ID3D11ShaderResourceView** out_srv, int* out_width, int* out_height)
{
	...
	MainWindow::g_pd3dDevice->CreateTexture2D(&desc, &subResource, &pTexture);
	...
	MainWindow::g_pd3dDevice->CreateShaderResourceView(pTexture, &srvDesc, out_srv);
	pTexture->Release();
...
std::expected<CTextureInfo, std::string> LoadTextureFromFile(const char* file_name)
{
	...
	CTextureInfo Ret{};
	LoadTextureFromMemory(file_data.data(), file_data.size(), &Ret.pTexture, &Ret.Width, &Ret.Height);
